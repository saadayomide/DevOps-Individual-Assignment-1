...
from database import Category as DBCategory, Ministry as DBMinistry, Proposal as DBProposal
...
class ContractParserService:
    ...
    def parse_contract(db: Session, file: UploadFile) -> Dict[str, List[Dict[str, Any]]]:
        ...
        # Mapping functions
        def map_category_id(category_name: Optional[str]) -> Optional[int]:
            ...
            category = db.query(DBCategory).filter(
                DBCategory.name.ilike(f"%{category_name}%")
            ).first()
            return category.id if category else None
        
        def map_ministry_id(ministry_name: Optional[str]) -> Optional[int]:
            ...
            # Try partial match
            ministry = db.query(DBMinistry).filter(
                DBMinistry.name.ilike(f"%{ministry_name}%")
            ).first()
            if ministry:
                return ministry.id
        ...
        def normalize_record(record: Dict[str, Any]) -> Dict[str, Any]:
            ...
            ministry_name = (
                record.get('ministry') or 
                record.get('dept') or 
                record.get('ministry_name')
            )
            category_name = (
                record.get('category') or 
                record.get('category_name') or 
                record.get('dept_category')
            )
            title = (
                record.get('title') or 
